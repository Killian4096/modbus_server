void modbus_server_func_08_diagnostics (struct modbus_server_t* modbus_server_tag){
    ...
    if (modbus_server_input_PDU_mapper_length(modbus_server_tag) < 3){
        modbus_server_exception_XX_illegal_function_length(modbus_server_tag);
        return;
    }

    diagnosic_subfunction = (modbus_server_input_PDU_mapper_get(modbus_server_tag, 1) << 8) | modbus_server_input_PDU_mapper_get(modbus_server_tag, 2); //Grab subfunction
...
void modbus_server_diagnostic_01_restart_comm_option (struct modbus_server_t* modbus_server_tag){
    ...
    uint16_t diagnosic_subfunction = (modbus_server_input_PDU_mapper_get(modbus_server_tag, 1) << 8) | modbus_server_input_PDU_mapper_get(modbus_server_tag, 2);
    ...
    clear_event_log = (modbus_server_input_PDU_mapper_get(modbus_server_tag, 3) << 8) | modbus_server_input_PDU_mapper_get(modbus_server_tag, 4);
...
void modbus_server_diagnostic_02_return_diagnostic_register (struct modbus_server_t* modbus_server_tag){
    ...
    modbus_server_output_PDU_mapper_add(modbus_server_tag, 8);
    modbus_server_output_PDU_mapper_add(modbus_server_tag, 0);
    modbus_server_output_PDU_mapper_add(modbus_server_tag, 2);
    modbus_server_output_PDU_mapper_add(modbus_server_tag, (modbus_server_tag->diagnostic_register)>>8);
    modbus_server_output_PDU_mapper_add(modbus_server_tag, modbus_server_tag->diagnostic_register);
    return;
}
...
void modbus_server_diagnostic_04_force_listen_only_mode (struct modbus_server_t* modbus_server_tag){
    ...
    if (modbus_server_input_PDU_mapper_get(modbus_server_tag, 3) || modbus_server_input_PDU_mapper_get(modbus_server_tag, 4)){
        modbus_server_exception_03_illegal_data_value(modbus_server_tag);
        return;
    }
